...
extern RC writeBlock(int currentPageNum, SM_FileHandle *sMFHandle, SM_PageHandle currentMemPage) {
    ...
    if (currentPageNum >= sMFHandle->totalNumPages) {
        fclose(filePointer);
        appendEmptyBlock(sMFHandle);
    }

    fwrite(currentMemPage, sizeof(char), PAGE_SIZE, filePointer);
    sMFHandle->curPagePos = ftell(filePointer);
    fclose(filePointer);
    return RC_OK;
}

...
extern RC openPageFile(char *fileName, SM_FileHandle *fHandle) {
    ...
    if (filePointer) {
        ...
        int tempTotalNumPages = bufferSize / PAGE_SIZE; 
...
extern RC readBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
    ...
    // Checking if the file has less than pageNum pages
...
extern RC writeBlock(int currentPageNum, SM_FileHandle *sMFHandle, SM_PageHandle currentMemPage) {
    ...
    if(currentPageNum < 0){
        statusMsg = RC_READ_NON_EXISTING_PAGE;
        return statusMsg;
    }
    ...
    if (fwrite(currentMemPage, sizeof(char), strlen(currentMemPage), filePointer) != strlen(currentMemPage)) {
        return RC_WRITE_FAILED;
    }

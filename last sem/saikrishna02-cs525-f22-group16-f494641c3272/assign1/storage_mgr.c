...
extern RC createPageFile ( char * fileName )
{
	...
	else 
    {
	    ...
	    if(PAGE_SIZE >= (fwrite(zeroPage, sizeof(char), PAGE_SIZE, filePointer)))
	    {
...
{   
    ...
    if(filePointer != 0)
    {
		...
		totalNumPages = ((ftell(filePointer) + 1) / PAGE_SIZE);// calculating number of pages using the filepointer end location and page size given.
		fHandle->totalNumPages = totalNumPages;// initializing total number of pages
...
RC readBlock (int pgNum, SM_FileHandle *fileHandle, SM_PageHandle mrPg)
		{
			...
			if(filePointer == NULL)
			...
			else
				{
					...
					(*fileHandle).curPagePos = pgNum; // Current page position update to pagenum 
				}
			fclose(filePointer);
			return RC_OK; 
		}
...
RC readFirstBlock (SM_FileHandle *fileHandle, SM_PageHandle mrPg)
{
    ...
    if(filePointer == NULL)
    ...
    else
        {
            ...
            (*fileHandle).curPagePos = 0; //First page of the file has index 0
        }
    fclose(filePointer);
    ...
    if (fHandle != NULL){
        ...
        if (pageNum >= 0 && pageNum < fHandle->totalNumPages){
            ...
            {   //Initially the filepointer has to moved the mentioned page number.
                if (fseek(filePointer, writeOffset, SEEK_SET) == 0) 
                    fwrite(mrPg, sizeof(char), PAGE_SIZE, filePointer);
                else    
                    return RC_WRITE_FAILED;
                    
                //Update the curPagePos in the file handle.
                fHandle->curPagePos = pageNum; 

                //Update the total number of pages in the file handle.
                //If more than one page of data is written the total number of pages has to upadted.

                //ftell gives the size of the file when pointer is at eof.

                fseek(filePointer, 0, SEEK_END);
                totalPage = ftell(filePointer) / PAGE_SIZE;

                fHandle->totalNumPages = totalPage; 

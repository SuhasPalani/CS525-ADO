...
int findLeastRecentlyUsedPage(BM_BufferPool *const bp)
{
    int index = -1;
    int leastNumber = INT_MAX;

    if (bp != NULL)  // Use NULL instead of nullptr in C
    {
        PageFrame *page_f = (PageFrame *)bp->mgmtData;
        ...
        for (int j = 0; j < buffer_size; ++j)
        {
            ...
            if (leastNumber > page_f[j].lru_num)
            {
                leastNumber = page_f[j].lru_num;
                index = j;
            }
        }
    }
...
// LRU function definition
extern void LRU(BM_BufferPool *const bp, PageFrame *pf)
{
    int index = findLeastRecentlyUsedPage(bp);
    int pgFrame = 1;

    PageFrame *page_f = NULL;  // Use NULL instead of nullptr in C
    int k = 0;

    // Check if buffer pool is not NULL
    if (bp != NULL)
    {
        pgFrame++;
    ...
    if (page_f != NULL && index >= 0 && page_f[index].modified == 1)
    {
    ...
    // Update the LRU number for the chosen page frame and increment counters.
    if (page_f != NULL && index >= 0)
    {
        page_f[index].lru_num = pf->lru_num, pgFrame = ++k;
